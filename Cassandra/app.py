...
def trades_view(session,username, tv_option):
    print_trade_history_menu()

    if tv_option == 1:
        model.get_trades_by_account(session, username)
    elif tv_option == 2:
        trade_type = input("Select Trade type(buy/sell): ").lower()
        model.get_trades_by_type(session,username,trade_type)
    elif tv_option == 3:
        symbol = input("Select symbol type: ").upper()
        trade_type = input("Select Trade type(buy/sell): ").lower()
        model.get_trades_by_symbol_type(session,username,symbol,trade_type)
    elif tv_option == 4:
        symbol = input("Select symbol type: ").upper()
        model.get_trades_by_symbol(session,username,symbol)
    else:
...
def main():
    ...
    while(True):
        ...
        if option == 2:
            model.get_position_by_accounts(session,username)
        if option == 3:
            print_trade_history_menu()
            tv_option = int(input('Enter your trade view choice: '))
            trades_view(session,username,tv_option)
        if option == 4:
            username = set_username()
        if option == 5:
            exit(0)
